(function() {
//  var BubbleChart, hankeUtil, campaignInit, root,
...
//            super_category: _this.get_supercategory(d.expenditure_category),
    ...
    BubbleChart.prototype.do_create_circles = function(circles) {
      var that;
      that = this;
			this.circles.enter().append("circle").attr("r", 0).style("fill", (function(_this) {
        return function(d) {
          return '#cfcfcf';
        };
...
//      if (category === 'Durable Assets' || category === 'Food & Beverages' || category === 'Insurance' || category === 'Lease/Rent' || category === 'teema Supplies' || category === 'Travel & Lodging' || category === 'Utilities' || category === 'Vehicle') {
//        return 'overhead';
//      } else if (category === 'Contribution to Community Organization' || category === 'Contribution to Political kunta' || category === 'Hawaii Election Campaign Fund') {
//        return 'contributions';
//      } else if (category === 'Advertising' || category === 'hanke Fundraiser Tickets' || category === 'Postage/Mailing' || category === 'Printing' || category === 'Surveys, Polls & Voter Lists') {
...
//        return d3.select(element).move_to_front();
    ...
    for (_i = 0, _len = allValuesArray.length; _i < _len; _i++) {
      ...
      switch (value) {
        case "Vaasa":
          ...
          break;
        case "EAKR TL 3":
          color_map[value] = '#77a1e5';
          break;
        case "ESR TL 1":
          color_map[value] = '#910000';
          break;
        case "ESR TL 2":
          color_map[value] = '#d62728';
          break;
        case "ESR TL 3":
          color_map[value] = '#d82f7e';
          break;
        case "ESR TL 4":
          color_map[value] = '#cc3300';
          break;
        case "MSR TL 1":
          color_map[value] = '#8bbc21';
          break;
        case "MSR TL 3":
          color_map[value] = '#a6c96a';
          break;
        case "MSR TL 4 (Leader)":
  ...
  BubbleChart.prototype.get_color_map_Lookup_Rahasto = function(allValuesArray) {
    ...
    for (_i = 0, _len = allValuesArray.length; _i < _len; _i++) {
      value = allValuesArray[_i];
      switch (value) {
        case "Aluekehitysrahasto":
          color_map[value] = '#2f7ed8';
          break;
        case "Sosiaalirahasto":
          color_map[value] = '#d62728';
          break;
        case "Maaseuturahasto":
          color_map[value] = '#8bbc21';
          break;
        default:
          color_map[value] = '#d62728';
  ...
  BubbleChart.prototype.get_color_map_lookup_set = function(allValuesArray) {
    ...
    baseArray = ["#98df8a", "#d62728", "#1f77b4", "#f7b6d2", "#7f7f7f", "#ffbb78", "#2ca02c", "#ff9896", "#9467bd", "#c5b0d5", "#8c564b", "#FFFF33", "#e377c2", "#aec7e8", "#ff7f0e"];
...
//      this.kill_forces();
//      $('#hanke_vis').find('svg').remove();
//      $('#hanke_modal').find('#expenditure-record-table-container').empty();
...
//      modal_viz_width = $('#hanke_vis').width();
//      viz = d3.select('#hanke_vis').append('svg').attr('width', '100%').attr('height', modal_viz_height);
...
//      var hanke_info, hanke, hanke_teema, cur_year, vuosi, encoded_category, modal, url, url_params, year;
//      console.log('updating modal');
//      url = 'https://data.hawaii.gov/resource/3maa-4fgr.json';
//      encoded_category = encodeURIComponent(category);
//      year = hanke_utils.get_vis_year();
//      vuosi = "" + (year - 2) + "-" + year;
//      url_params = "$limit=20&$where=reg_no='" + reg_no + ("'and expenditure_category='" + encoded_category + "' and vuosi = '") + vuosi + "'&$order=amount desc";
//      modal = $('#hanke_modal');
...
//          }
//        ];
//       return tabulate('#hanke_modal #expenditure-record-table-container', 'expenditure-record-table', data, columns);
//     }).always(function() {
//        return modal.find('.expenditure-loading').hide();
//      }).error(function() {
//        return modal.find('.expenditure-error').show();
//      });
//      hanke_info = window.organizational_records.filter(function(d) {
//        return d.reg_no === reg_no;
//      })[0];
//      hanke = hanke_info.hanke;
//      hanke_teema = hanke_info.teema;
//      modal.find('.hanke').text(hanke);
//      cur_year = hanke_utils.get_vis_year();
//      modal.find('.current_year').text(cur_year);
//      modal.find('.hanke_teema').text(hanke_teema);
...
//    hankeUtil.prototype.get_island = function(record) {
...
//      };
//      if ((_ref = record.teema) === 'Governor' || _ref === 'Mayor' || _ref === 'Lt. Governor' || _ref === 'Prosecuting Attorney' || _ref === 'OHA' || _ref === 'BOE') {
//        return 'All';
//      } else if ((_ref1 = record.teema) === 'Honolulu Council') {
//        return 'Oahu';
//      } else if ((_ref2 = record.teema) === 'Maui Council') {
//        return maui;
//      } else if ((_ref3 = record.teema) === 'Kauai Council') {
//        return kauai;
//      } else if ((_ref4 = record.teema) === 'Hawaii Council') {
//        return 'Hawaii';
//      } else if ((_ref5 = record.teema) === 'Senate') {
...
//      } else if ((_ref6 = record.teema) === 'House') {
//        matches = window.precinct_records.filter(function(d) {
...
//      };
//		})(this);
    return queue().defer(d3.csv, "data/kirjastohankkeet.csv").await(render_vis);
  });
